 ...
 * \note QVariant — это универсальный тип данных Qt, позволяющий хранить
...
QVariant Notebook::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole)
    {
        ...
        QSettings settings("SiberianUniversityProject","Toynote");                  //Get value of setting "Font" it is a QVariant
        QFont font = qvariant_cast<QFont>(settings.value("Font"));                  //qvariant_cast to QFont the nreturn font
        return font;
    }
...
void Notebook::setNote(Notebook::SizeType idx, Note note)
{
    // Присваиваем заметку
    mNotes[idx] = note;
    // В соответствии с требованиями Qt, уведомляем привязанные виды о том,
